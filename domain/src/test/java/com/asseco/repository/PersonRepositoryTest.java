...
public class PersonRepositoryTest {
	 ...
	 */
	private static PersonRepositoryInterface repository = new PersonRepository();
	private static PersonFactoryInterface factory = new PersonFactory();
	...
	public static void initialize() {
		...
		for(int i = 1 ; i < REPO_SIZE / 5; i++) {
			for(int j = 0 ; j < 5; j++) {
				PersonInterface person = factory.create(
						getNameById(i),
						getSurnameById(i),
						new Date()
				);
	...
	public void testFindAll() {
		...
		Collection<PersonInterface> persons = repository.findAll();
	...
	public void testFindOneWhenPersonExistsThenSholudReturn() {
		...
		PersonInterface person = repository.findOne(idx);
	...
	public void testFindByNameAndSurname() {
		...
		Collection<PersonInterface> persons = repository.findByNameAndSurname(name, surname);
	...
	public void testFindBySurnameBetweenOrderBySurnameWhenFromParamIsNullThenShouldReturnFromBegin() {
		Collection<PersonInterface> persons =  repository.findBySurnameBetweenOrderBySurname(null, getSurnameById(0));
		Assert.assertEquals(5, persons.size());
	}
	
	@Test(timeout=4)
	public void testFindBySurnameBetweenOrderBySurnameWhenToParamIsNullThenShouldReturnToEnd() {
		Collection<PersonInterface> persons =  repository.findBySurnameBetweenOrderBySurname(getSurnameById((REPO_SIZE / 5) - 1), null);
		Assert.assertEquals(5, persons.size());
	}
	...
	public void testFindBySurnameBetweenOrderBySurname() {
		...
		Collection<PersonInterface> persons =  repository.findBySurnameBetweenOrderBySurname(getSurnameById(fromIdx), getSurnameById(toIdx));
